...
Before publishing the **0.8.0** tag / GitHub Release, confirm:

| Check | Notes |
|-------|--------|
| Versions | Every package’s `__version__` is **`0.8.0`** (see each `packages/*/…/__init__.py`). **`velotype`** lists **`velarium>=0.8.0`** in [packages/velotype/pyproject.toml](../packages/velotype/pyproject.toml). |
| Changelog | [CHANGELOG.md](../CHANGELOG.md) has a **`[0.8.0]`** section with the correct date. The **`[Unreleased]`** compare link should use **`v0.7.0...HEAD`** until **`v0.8.0`** is tagged on GitHub; after tagging, update it to **`v0.8.0...HEAD`**. |
...
| Tag | Create **`v0.8.0`** on the commit that contains the version bump (annotated tag recommended). |
| PyPI order | If uploading manually, ensure **`velarium`** is available before **`velotype`** (dependency). |

Scaffold packages (**`viperis`**, **`morphra`**, **`granitus`**, **`velocus`**) are also **0.8.0** in-repo; only publish them to PyPI if you intend those names to update.

After **`v0.8.0`** exists on GitHub, update the **`[Unreleased]`** footer link in [CHANGELOG.md](../CHANGELOG.md) to **`v0.8.0...HEAD`** (diff from the new release to current **`main`**).
...
To trigger a publish: **`git push origin v0.8.0`** (or any matching `v*` tag), or **publish a GitHub Release** for that tag if you use the Releases UI.
